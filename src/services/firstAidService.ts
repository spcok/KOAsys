...
export const firstAidService = {
  saveLog: async (data: Partial<FirstAidLog>, userId?: string): Promise<void> => {
    ...
    await baseService.upsertCollection(firstAidLogsCollection, payload as unknown as any);
  ...
  getFirstAidLogs: async (): Promise<FirstAidLog[]> => {
    const list = Array.from(firstAidLogsCollection.values()) as unknown as FirstAidLog[];
